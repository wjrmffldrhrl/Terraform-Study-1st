...
잠그기
대부분의 원격 백엔드는 선천적으로 잠그는 기능을 제공한다. 한 유저가 apply를 하면 테라폼은 잠기는 상태가 되어 다른 유저가 apply를 하면 적용되지 않는다.

보안유지
대부분의 원격 백엔드는 암호화 기능을 제공한다. 텍스트 형식의 상태파일들이 암호화가 되어 저장되면 보안을 유지할 수 있다.

만약 AWS 유저라면 S3를 원격 백엔드로 사용하는 것이 제일 좋은 방법이다.
* S3는 AWS로 인해 관리되는 서비스라 추가 인프라 구축을 할 필요가 없다
* 데이터 손실을 걱정할 필요가 없다
* 암호화를 지원하기 때문에 보안 데이터를 보호할 수 있다
* 잠금 기능을 제공한다
* 버전컨트롤이 가능하다
* 싸다!

S3를 원격 상태 저장소로 활용하려면 S3 버킷을 생성해야한다. 새로운 main.tf 파일을 만들어 하단과 같이 구성해준다.
...
위의 코드는 버킷의 이름을 설정해주고 버킷이 실수로 삭제되는 것을 방지해준다.
